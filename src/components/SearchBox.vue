...
<template>
  <div class="w-full max-w-2xl mx-auto relative z-10">
    <!-- Backdrop for closing dropdown (放在最外层) -->
    <div 
      v-if="isDropdownOpen" 
    ...
    <div 
      v-if="isDropdownOpen"
      class="absolute top-full left-0 mt-2 w-48 py-2 bg-slate-900/90 border border-white/10 rounded-xl backdrop-blur-xl shadow-2xl overflow-hidden origin-top-left z-50"
    >
      <button
    ...
    >
      ...
      <button 
        ...
        class="absolute left-0 top-0 bottom-0 z-30 h-full px-4 rounded-l-2xl text-white/70 hover:text-white hover:bg-white/10 transition-colors flex items-center gap-2 cursor-pointer font-medium"
